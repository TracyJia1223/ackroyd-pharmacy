...
function doPost(e) {
  try {
    ...
    if (data.formType === 'refill') {
      ...
      subject = '[Ackroyd Pharmacy] Refill Request – ' + data.firstName + ' ' + data.lastName;
      ...
      subject = '[Ackroyd Pharmacy] Transfer Request – ' + data.firstName + ' ' + data.lastName;
      ...
      subject = '[Ackroyd Pharmacy] Contact Message – ' + data.firstName + ' ' + data.lastName;
...
function fmt(title, fields, ts) {
  ...
  lines += '\nSource   : Ackroyd Pharmacy Website';
  return lines;
}
